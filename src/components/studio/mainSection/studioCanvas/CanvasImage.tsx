...
import { setCanvasReady, setIsResetZoom, setZoom } from "@/hooks/slices/canvas/canvasSlice";
import { LoadImageWithCORS, LoadImageWithFetch, setBackgroundImage } from "../../canvasUtils/canvasImageUtils";
// import {
...
const CanavasImage = forwardRef(
  (
    ...
    const { activeCanvas, isCanvasIcon } = useSelector(
    ...
    const handleMouseMove = useCallback(
      (event: fabric.TEvent) => {
        ...
        if (!fabricCanvas) {
        // const fabricCanvas = fabricCanvasRef.current;
        //   console.log("in edit annotation mode or fabricCanvas not ready", fabricCanvasRef.current);
          return;
        }
    ...
    const handleMouseWheel = (event: fabric.TEvent) => {
      ...
      const pointer = fabricCanvasRef.current?.getScenePoint(event.e);
      ...
      const tryLoadImage = async () => {
        ...
        for (const corsMode of corsOptions) {
          try {
            ...
            setBackgroundImage(
              fabricCanvasRef as React.RefObject<fabric.Canvas>,
              imageUrl,
              backgroundImageRef,
              (loading: boolean) => {
    ...
    return (
      <>
        ...
        <TooltipProvider>
          <div
            className={cn(
              "w-full h-full flex flex-col mb-3 transition-all duration-300 ease-in-out p-4",
              className
            )}
          >
            {/* Canvas Container */}
